...
開一條走道的成本是揀一個棧板的 1.74 倍。因此，最小化總搬運時間可化約為最小化走道切換次數。
...
| | |
|---|---|
| **DHGA 能展開較完整的取捨** | DHGA 在偏好掃描下產出 **12.4** 個非支配解,HGA 7.1、Greedy 2.2、GA 1.3。Greedy 的兩個目標相關係數達 +0.99，幾乎看不到取捨空間。。 |
| **差距隨問題規模擴大** | 相對次佳方法的 Hypervolume 改善,由 288 棧板時的 **+2.9%** 升至 672 棧板時的 **+6.3%**,九個情境結論一致。 |
| **執行時間沒有比較慢** | DHGA 平均 94.0 秒，對照方法為 99.3 與 101.3 秒，約快 5.3%，前緣也較完整。 |
| **結果接近理論下限** | 在附帶的 288 棧板實例中,解只切換 **23 次**走道,硬下界為 20 次——僅高出 **15%**。 |
...
**停滯偵測與 Greedy 注入。** 當最佳目標值連續 $G_{stall}$ 代未改善時,以走道分群啟發式建構的解替換部分非菁英個體。這會把搜尋重新導向已依走道群聚的序列。
